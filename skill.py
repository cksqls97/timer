...
def start_logic(names):
    ...
    def create_overlay():
        ...
        def make_card(parent, k, is_guest=False):
            ...
            if not is_guest:
                for w_ in [c, nl, tl, msg]:
                    w_.bind("<Button-3>", lambda e, x=k: toggle_res(x))
            return c, nl, tl, msg
    ...
    def update_display():
        ...
        for rk in ['f1','f2','f3','f4']:
            ...
            if nt_times[rk] and nt_times[rk] > now:
                ...
                warn_texts = []
                # --- [고급 시뮬레이션: 손님 부활 즉시 재사망 가정] ---
                
                # 1. 사용 시 시뮬레이션
                # 내가 지금 리저를 씀 -> 손님이 즉시 또 죽음 -> 남은 비숍 중 가장 빠른 리저가 마지노선 이내인가?
                tmp_u = sorted(alive_times)
                try:
                    idx = tmp_u.index(now)
                    tmp_u[idx] = now + datetime.timedelta(minutes=30) # 나 사용 완료
                    tmp_u = sorted(tmp_u)
                    # 손님이 살자마자 죽었다고 가정하므로, "다음 순번 비숍"이 바로 리저를 썼을 때 그다음 쿨이 있는가?
                    if len(tmp_u) >= 2:
                        tmp_u[0] = tmp_u[0] + datetime.timedelta(minutes=30) # 다음 주자 사용
                    
                    next_res = min(tmp_u)
                    if next_res > guest_deadline: warn_texts.append("사용 시 불가")
                except: pass

                # 2. 사망 시 시뮬레이션
                # 내가 지금 죽음 -> 남은 사람 중 한 명이 손님을 살림 -> 손님이 즉시 또 죽음 -> 그다음 주자가 있는가?
                tmp_d = sorted(alive_times)
                try:
                    tmp_d.remove(now) # 나 제외
                    if tmp_d:
                        tmp_d[0] = tmp_d[0] + datetime.timedelta(minutes=30) # 다음 주자가 살림
